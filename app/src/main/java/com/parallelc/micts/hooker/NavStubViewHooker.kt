...
class NavStubViewHooker {
    companion object {
        ...
        fun hook(param: PackageLoadedParam, skipHookTouch: Boolean) {
            val navStubView = param.defaultClassLoader.loadClass("com.miui.home.recents.NavStubView")
            runCatching {
                module!!.hook(navStubView.getDeclaredMethod("startRecentsAnimationPre"))
                    .intercept(SkipHooker())
            }
        ...
        class OnTouchEventHooker : Hooker {
            ...
            override fun intercept(chain: Chain): Any? {
                ...
                runCatching {
                    ...
                    module!!.log(Log.ERROR, "NavStubViewHooker", "onTouchEvent fail", e)
                }
                return result
            }
        }
        ...
        class ConstructorHooker : Hooker {
            override fun intercept(chain: Chain): Any? {
                val result = chain.proceed()
                mContext = WeakReference(chain.getArg(0) as Context)
                return result
            }
        }
    }
