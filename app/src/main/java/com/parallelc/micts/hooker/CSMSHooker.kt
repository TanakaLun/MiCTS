...
class CSMSHooker {
    companion object {
        ...
        fun startContextualSearch(entryPoint: Int): Boolean {
            var unhookers = mutableListOf<HookHandle>()
            return runCatching {
                unhookers += module!!.hook(enforcePermission!!).intercept(EnforcePermissionHooker())
                unhookers += module!!.hook(getContextualSearchPackageName!!).intercept(GetCSPackageNameHooker())
        ...
        class DeviceHasConfigStringHooker : Hooker {
            override fun intercept(chain: Chain): Any? {
                if (chain.getArg(1) == contextualSearchPackageName) {
                    return true
                }
