...
class NavBarEventHelperHooker {
    companion object {
        ...
        fun hook(param: PackageLoadedParam) {
            val navStubGestureEventManager = param.defaultClassLoader.loadClass("com.miui.home.recents.cts.NavBarEventHelper")
            mContext = navStubGestureEventManager.getDeclaredField("mContext")
            mContext.isAccessible = true
            module!!.hook(navStubGestureEventManager.getDeclaredMethod("onLongPress", MotionEvent::class.java))
