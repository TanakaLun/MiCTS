...
class NavStubGestureEventManagerHooker {
    companion object {
        ...
        fun hook(param: PackageLoadedParam) {
            val navStubGestureEventManager = param.defaultClassLoader.loadClass("com.miui.home.recents.gesture.NavStubGestureEventManager")
            getInstance = runCatching {
                param.defaultClassLoader.loadClass("com.miui.home.launcher.Application").getDeclaredMethod("getInstance")
            }.getOrNull()
            module!!.hook(navStubGestureEventManager.getDeclaredMethod("handleLongPressEvent"))
                .intercept(HandleLongPressEventHooker())
        }
        @XposedHooker
